...
async function main() {
  ...
  const redisSkuName =
    config.get("redisSkuName") || (redisServiceType === "managed" ? "Enterprise" : "Standard");
  const redisSkuFamily = config.get("redisSkuFamily") || "C";
  const redisSkuCapacity = parseInt(
    config.get("redisSkuCapacity") || (redisServiceType === "managed" ? "10" : "1")
  );
