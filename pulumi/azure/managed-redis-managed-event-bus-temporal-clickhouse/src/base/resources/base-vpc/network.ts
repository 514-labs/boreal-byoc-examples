...
export async function createNetwork(args: NetworkArgs) {
  ...
  // Temporarily without delegation to avoid triggering VNet integration requirement
  const apiServerSubnet = new azure.network.Subnet("api-server-subnet", {
    ...
    // Delegation commented out - only needed when using API Server VNet Integration
    // delegations: [{
    //   name: "aks-delegation",
