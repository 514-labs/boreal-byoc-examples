...
import { createAksCluster } from "./resources/aks";
...
async function main() {
  ...
  const { aksCluster } = await createAksCluster({
  ...
  // which is configured as an adminGroupObjectID via az aks update command
  // All Engineering group members automatically have cluster-admin permissions
  // Kubeconfig - use user credentials (AAD-based) since AKS Automatic disables local accounts
  const creds = azure.containerservice.listManagedClusterUserCredentialsOutput({
    resourceGroupName: resourceGroup.name,
    resourceName: aksCluster.name,
  });

  const kubeconfig = creds.kubeconfigs[0].value.apply((v: string) =>
    Buffer.from(v, "base64").toString()
  );

  // NOTE: Kubernetes resources are managed separately via kubectl/helm
  // The kubeconfig is exported and can be used with:
  // pulumi stack output kubeconfig --stack base > kubeconfig.yaml
  // export KUBECONFIG=kubeconfig.yaml
  // kubectl apply -f your-manifests.yaml

  // This separation is actually a best practice:
  // - Pulumi manages infrastructure (cluster, network, RBAC)
  // - K8s-native tools manage workloads (kubectl, helm, ArgoCD)

  // If you need to manage K8s resources in Pulumi later, uncomment below:
